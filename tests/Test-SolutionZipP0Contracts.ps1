...
try {
    ...
    Add-Check "ListarTarefas accepts NomeProjeto" (($listar -match '"NomeProjeto"') -and ($listar -match "Compose_ProjectInput")) "P0: flow trigger must accept a project name."
    $usesCanonicalProjectId = $listar.Contains("body('Get_Projeto')?['value']?[0]?['ProjectID']") -or $listar.Contains("body(\u0027Get_Projeto\u0027)?[\u0027value\u0027]?[0]?[\u0027ProjectID\u0027]")
    ...
    Add-Check "ListarTarefas has not-found guard" (($listar -match "Condition_Projeto_Encontrado") -and ($listar -match "PROJECT_NOT_FOUND")) "P0: invalid project names must not silently list empty tasks."
    ...
    Add-Check "ListarTarefas action binds NomeProjeto" ($listarAction -match "propertyName:\s*NomeProjeto") "Action must pass NomeProjeto into flow."
}
finally {
