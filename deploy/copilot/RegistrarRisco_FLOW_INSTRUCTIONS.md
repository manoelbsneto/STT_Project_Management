# RegistrarRisco flow instructions

Flow display name: `PMO_PA_RegistrarRiscoBot`

Purpose: validate a project and create one item in SharePoint list `Riscos e Bloqueios` with `Tipo` fixed as `Risco`.
...
```text
concat('RSK-', formatDateTime(utcNow(), 'yyyyMMddHHmmss'))
```
...
Use the `ProjectID` trigger value through dynamic content. If the designer requires an expression, build the filter with `concat(...)`.
...
```text
Risco @{variables('varRiskID')} registrado para o projeto @{triggerBody()?['text']}.
```
...
```text
Projeto nao encontrado ou inativo. Nenhum risco foi registrado.
```
...
- Copy the flow id from the flow URL and replace `REPLACE_WITH_ACTUAL_FLOW_ID` in `RegistrarRisco_topic.yaml`.
