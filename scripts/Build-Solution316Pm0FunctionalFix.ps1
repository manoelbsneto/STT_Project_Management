...
function Ensure-BotComponentWorkflowMappingXml {
    param(
        [string]$WorkflowSetXml,
        [string]$ActionSchemaName,
        [string]$WorkflowId
    )

    if ($WorkflowSetXml -match [regex]::Escape($ActionSchemaName)) {
        return $WorkflowSetXml
    }

    $entry = @"
  <botcomponent_workflow botcomponentid.schemaname="$ActionSchemaName" workflowid.workflowid="$WorkflowId">
    <iscustomizable>1</iscustomizable>
  </botcomponent_workflow>
"@

    return ($WorkflowSetXml -replace "(\s*</botcomponent_workflowset>)", "`r`n$entry`r`n</botcomponent_workflowset>")
}
...
$workflowSetPath = Join-Path $workingFullPath "Assets\botcomponent_workflowset.xml"
$workflowSetXml = Get-Content -LiteralPath $workflowSetPath -Raw
...
foreach ($flow in $flows) {
    ...
    $workflowSetXml = Ensure-BotComponentWorkflowMappingXml `
        -WorkflowSetXml $workflowSetXml `
        -ActionSchemaName "pmo_AssistentePMO_V2.action.$($flow.action)" `
        -WorkflowId $flow.id
}
...
Write-TextFile -Path $workflowSetPath -Content $workflowSetXml
